...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function searchPokemonStats() {
      ...
      try {
          // First try to find by exact name or ID
          const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${searchTerm}`);
          const pokemon = await response.json();
          
          // Get species data for type effectiveness
          ...
          statsResults.innerHTML = '<p>Pokémon não encontrado. Tente outro nome ou número.</p>';
          
          // Clear charts
          if (baseStatsChart) baseStatsChart.destroy();
          if (typeEffectivenessChart) typeEffectivenessChart.destroy();
      }
  }
  ...
  function createBaseStatsChart(stats) {
      ...
      // Create new chart
      baseStatsChart = new Chart(baseStatsChartCanvas, {
          type: 'bar',
          data: chartData,
  ...
  // Initialize stats section
  function initStats() {
      // Set placeholder with random Pokémon
